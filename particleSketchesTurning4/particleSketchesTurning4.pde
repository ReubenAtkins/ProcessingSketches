...
//  image(img, 0, 0);
 ...
 noStroke();
  for(int i = p.size()-1; i >= 0; i--) {
    particles t = p.get(i);
    t.update();
   if(t.location() < img.pixels.length-1 && t.location() > 0 && i%444 != 0) {
...
class particles {
  ...
  particles() {
    loc = new PVector (.5*width+randomGaussian()*40, .5*height+randomGaussian()*40);
    ploc = loc.copy();
  ...
  void update() {
    ...
    float temp = PVector.angleBetween(centNorm, vel);
        //println(vel.x+" "+vel.y);
   // println(temp);
    ploc = loc.copy();
    //vel = 
